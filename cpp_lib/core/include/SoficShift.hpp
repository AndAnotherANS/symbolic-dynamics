...
template <bool RightResolving, bool Irreducible> class SoficShift : public ShiftSpace
{
  protected:
...
inline SoficShift<RR, IRR>::SoficShift(std::vector<unsigned int> alphabet, UnweightedMatrixGraph edge_shift)
    : edge_shift(edge_shift), ShiftSpace(alphabet){};
...
SoficShift<RR, false> inline sofic_shift_union(SoficShift<RR, IRR> ss1, SoficShift<RR, IRR> ss2)
{
    ...
    for (auto &&a : ss1.get_alphabet())
        alphabet.insert(a);
    for (auto &&a : ss2.get_alphabet())
        alphabet.insert(a);
    ...
    for (auto &edge1 : ss1.get_edge_shift().edges())
    {
        for (auto &edge2 : ss2.get_edge_shift().edges())
        {
            if (edge1.label != edge2.label)
                continue;
            label_product_graph.add_edge(edge1.source * size1 + edge2.source, edge1.dest * size1 + edge2.dest, 1,
                                         edge1.label);
        }
    }
...
template <bool IRR> class SoficShift<true, IRR> : ShiftSpace
{
  protected:
    UnweightedMatrixGraph edge_shift;
...
template <bool IRR> using RightResolvingSoficShift = SoficShift<true, IRR>;
